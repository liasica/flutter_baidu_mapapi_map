...
class BMFHeatMap implements BMFModel {
  ...
  Map<String, Object?> toMap() {
    return {
      ...
      'datas': this.datas?.map((list) => list.map((node) => node.toMap()).toList()).toList(),
  ...
  BMFHeatMap.fromMap(Map map) {
    ...
    animation = map['animation'] == null ? null : BMFAnimation.fromMap(map['animation']);
    frameAnimation = map['frameAnimation'] == null ? null : BMFAnimation.fromMap(map['frameAnimation']);
  }
...
class BMFGradient implements BMFModel {
  ...
  @override
  Map<String, Object?> toMap() {
    return {'colors': this.colors?.map((color) => color.value.toRadixString(16)).toList(), 'startPoints': this.startPoints?.map((p) => p).toList()};
  }
}
